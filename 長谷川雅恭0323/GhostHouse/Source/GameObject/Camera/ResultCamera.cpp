...
void ResultCamera::OnUpdate(float deltaTime)
{
	m_Rotate.identity();

	m_Position = GSvector3(0.0f, 0.0f, 0.0f);

	static GSvector3 l_Back = GSvector3(30.0f, 0.0f, -10.0f);
	const GSvector3 l_Up = GSvector3(0.0f, 10.0f, 0.0f);

	m_Position += l_Back + l_Up;

	m_Rotate.rotateY(-15.0f);
	m_Rotate.rotateX(5.0f);
}

void ResultCamera::OnDraw(bool is_shadow, Renderer & renderer) const
{
